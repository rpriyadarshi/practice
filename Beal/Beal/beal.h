...
namespace Beal {
    ...
    // Size factor between different data types
    ...
    constexpr const unsigned long ByteBits              = 8;
    
    // Calculate number of bits for some data types
    constexpr const unsigned long UShortBits            = sizeof(unsigned short) * ByteBits;
    constexpr const unsigned long ULongBits             = sizeof(unsigned long) * ByteBits;
    constexpr const unsigned long IntBits               = sizeof(int) * ByteBits;
    
    // long is biggest integer. We want BitInt to be larger than long by the factor given below.
    constexpr const unsigned long BigIntLongFact        = 4;
    
    // ULongToUShort is the number of shorts needed to fill up a long.
    constexpr const unsigned long BigIntUShortFact      = ULongToUShort * BigIntLongFact;
    
    // We need double sized space to hold results of multiplication.
    constexpr const unsigned long BigIntUShortMultFact  = ULongToUShort * (BigIntLongFact * 2);
    ...
    public:
        ...
        size_t size() const                             { return BigIntUShortFact; }
        size_t dataSize() const                         { return BigIntUShortMultFact; }
    ...
    private:
        unsigned short  _data[BigIntUShortMultFact];
    };
